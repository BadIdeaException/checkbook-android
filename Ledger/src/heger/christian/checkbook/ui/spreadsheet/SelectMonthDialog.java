...
public class SelectMonthDialog extends DialogFragment implements OnValueChangeListener {
	...
	private void adjustPickerBounds(int year) {
		...
		// Need to adjust displayValues because the NumberPicker will ALWAYS show from index 0
		// So even if minVal == FEBRUARY and the picker is set to 1 (i.e. what should actually be
		// FEBRUARY), it'll show JANUARY instead (it internally does displayNames[value-minValue])
		String[] displayValues;
		if (minVal > Calendar.JANUARY) {
			displayValues = Arrays.copyOfRange(monthNames, minVal, monthNames.length);
		} else {
			displayValues = monthNames;
		}
		pickMonth.setDisplayedValues(displayValues);
