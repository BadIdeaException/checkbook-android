 ...
 * in <code>CheckbookDbHelper</code> is <i>only</i> guaranteed to provide the
...
public abstract class EntryContract implements BaseColumns {
	...
	protected static String mapToDBContract(String in) {
		if (in.equals(TABLE_NAME))
			return CheckbookDbHelper.EntryContract.TABLE_NAME;
		if (in.equals(_ID))
			return CheckbookDbHelper.EntryContract._ID;
		if (in.equals(COL_NAME_CAPTION))
			return CheckbookDbHelper.EntryContract.COL_NAME_CAPTION;
		if (in.equals(COL_NAME_CATEGORY))
			return CheckbookDbHelper.EntryContract.COL_NAME_CATEGORY;
		if (in.equals(COL_NAME_DETAILS))
			return CheckbookDbHelper.EntryContract.COL_NAME_DETAILS;
		if (in.equals(COL_NAME_DATETIME))
			return CheckbookDbHelper.EntryContract.COL_NAME_DATETIME;
		if (in.equals(COL_NAME_VALUE))
			return CheckbookDbHelper.EntryContract.COL_NAME_VALUE;
		return null;
	}
